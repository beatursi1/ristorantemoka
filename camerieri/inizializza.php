...
// Info cameriere dalla sessione (non più dalla tabella camerieri)
$cameriere_id   = $_SESSION['utente_id'];
$cameriere_nome = $_SESSION['utente_nome'];
$cameriere_codice = $_SESSION['utente_ruolo'] === 'admin' ? 'ADMIN' : 'CAM';

// Gestione modalità: 'manage' (default) o 'order'
$mode = isset($_GET['mode']) ? $_GET['mode'] : 'manage';

// Query filtrata: se siamo in modalità ordine, mostra solo i tavoli occupati
$sql = "SELECT id, numero, stato FROM tavoli";
if ($mode === 'order') {
    $sql .= " WHERE stato = 'occupato'";
}
$sql .= " ORDER BY CAST(numero AS UNSIGNED) ASC";
$tavoli = $conn->query($sql);
?>
...
<body data-cameriere-id="<?php echo $cameriere_id; ?>">
    ...
    <div class="header mb-4">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-8">
                    <h1 class="h4 mb-0"><i class="fas <?php echo $mode === 'order' ? 'fa-utensils' : 'fa-users-cog'; ?> me-2"></i><?php echo $mode === 'order' ? 'Ordina per Cliente' : 'Gestione Tavoli'; ?></h1>
                    <small>Cameriere: <?php echo htmlspecialchars($cameriere_nome); ?> (<?php echo htmlspecialchars($cameriere_codice); ?>)</small>
                </div>
                <div class="col-4 text-end">
                    <?php 
                    $dashUrl = $_SESSION['utente_ruolo'] === 'admin' 
                        ? '/ristorantemoka/admin/dashboard.php' 
                        : '/ristorantemoka/camerieri/dashboard.php';
                    ?>
                    <a href="<?php echo $dashUrl; ?>" class="btn btn-outline-light btn-sm me-2">
                    ...
                    <?php endif; ?>
                    <a href="/ristorantemoka/logout.php" class="btn btn-light btn-sm">Logout</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
                    ...
                    <div class="col-md-3 col-6 mb-3">
                        <div class="card tavolo-card text-center p-3 
                            ...
                            <div class="mt-2 small border-top pt-2">
                                <?php
                                $tId = (int)$tavolo['id'];
                                $resC = $conn->query("SELECT nome_cliente, identificativo FROM sessioni_clienti WHERE tavolo_id = $tId");
                                $cNames = [];
                                while($c = $resC->fetch_assoc()) { 
                                    $cNames[] = $c['nome_cliente'] ?: "Cliente " . $c['identificativo']; 
                                }
                                echo !empty($cNames) 
                                    ? '<span class="text-dark fw-bold">' . htmlspecialchars(implode(", ", $cNames)) . '</span>' 
                                    : '<span class="text-muted fst-italic">Nessun cliente seduto</span>';
                                ?>
                            </div>
                            <?php endif; ?>
                        </div>
        ...
        <div class="card mb-4" id="fase2" style="display: none;">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <!-- COLONNA DESTRA: CONFIGURAZIONE CLIENTI -->
                    <div class="col-md-6">
                        ...
                        <div id="clienti-extra-container" class="row mb-4"></div>
                    </div>
                </div>
                ...
                <div class="text-center mt-4">
                    ...
                    <p class="text-muted small mt-2">Procedi dopo che i clienti si sono registrati</p>
                </div>
            </div>
        </div>

        <!-- FASE 3: BEVANDE INIZIALI -->
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="d-flex align-items-center justify-content-center mb-3">
                            <button class="btn btn-outline-primary btn-lg" id="btn-acqua-meno"><i class="fas fa-minus"></i></button>
                            <div class="mx-4 text-center">
                                <div class="h2 mb-0" id="contatore-acqua">0</div>
                                <small class="text-muted">bottiglie da 1L</small>
                            </div>
                            <button class="btn btn-outline-primary btn-lg" id="btn-acqua-piu"><i class="fas fa-plus"></i></button>
                        </div>
        ...
        <!-- FASE 4: CONFERMA E FINALIZZAZIONE -->
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div id="riepilogo-clienti" class="mb-3"><p><em>Caricamento clienti...</em></p></div>
                        <div id="riepilogo-bevande" class="mb-3"><p><em>Nessuna bevanda</em></p></div>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="select-lettera-extra">
                            <?php foreach(range('G', 'Z') as $lettera): ?>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nome-cliente-extra" placeholder="Es: Marco, Anna...">
                    </div>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="contatore-quantita">
                            <button class="btn btn-outline-secondary" id="btn-bevanda-meno"><i class="fas fa-minus"></i></button>
                            <div class="text-center">
                                <span class="h4 mb-0" id="quantita-bevanda">1</span>
                            </div>
                            <button class="btn btn-outline-secondary" id="btn-bevanda-piu"><i class="fas fa-plus"></i></button>
                        </div>
                        <div class="text-center mt-2">
                            <small class="text-muted">Totale: €<span id="prezzo-totale-bevanda">18.00</span></small>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="cond-personale"><i class="fas fa-user me-1"></i>Solo per chi ordina</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="condivisione-bevanda" id="cond-tavolo" value="tavolo">
                            <label class="form-check-label" for="cond-tavolo"><i class="fas fa-users me-1"></i>Diviso per tutto il tavolo</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="condivisione-bevanda" id="cond-parziale" value="parziale">
                            <label class="form-check-label" for="cond-parziale"><i class="fas fa-user-friends me-1"></i>Scegli chi partecipa</label>
                        </div>
                    </div>
                    <div class="mb-3" id="selettore-partecipanti" style="display: none;">
                        <label class="form-label">Seleziona partecipanti</label>
                        <div id="checkbox-partecipanti" class="border rounded p-3" style="max-height: 200px; overflow-y: auto;"></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annulla</button>
    ...
    <!-- Modal seleziona cliente manuale -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div id="lista-clienti-manuali" class="row"></div>
    ...
    <script>
        window.isCameriere = true;
        window.cameriereId = <?php echo $cameriere_id; ?>;
    </script>
</body>
</html>
<?php $conn->close(); ?>