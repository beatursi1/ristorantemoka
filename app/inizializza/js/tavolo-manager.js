...
class TavoloManager {
    ...
    async onTavoloClick(event) {
        ...
        this.tavoloSelezionato = {
            ...
            sessione_token: null // Sarà popolato dall'API
        };

        const statoAttuale = card.dataset.stato;

        // Se già occupato: recupera solo il token esistente senza modificare nulla
        // Se libero: occupa il tavolo e crea nuova sessione pulita
        ...
        try {
            ...
            if (result.success && result.sessione_token) {
                this.tavoloSelezionato.sessione_token = result.sessione_token;
                console.log(
                    statoAttuale === 'occupato'
                        ? 'Tavolo già occupato — sessione recuperata: ' + result.sessione_token
                        : 'Nuovo tavolo occupato — sessione creata: ' + result.sessione_token
                );
