...
class TavoloManager {
    ...
    async onTavoloClick(event) {
        ...
        // Chiama API per aggiornare stato nel DB
        try {
            ...
            if (result.success && result.sessione_token) {
                // FIX TREMENDO: Salviamo il token reale della sessione del tavolo.
                // Questo permette al cameriere di vedere gli ordini di Anx.
                this.tavoloSelezionato.sessione_token = result.sessione_token;
                console.log("Sincronizzazione sessione tavolo riuscita:", result.sessione_token);
