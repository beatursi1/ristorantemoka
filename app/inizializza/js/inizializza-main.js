class InizializzaApp {
    ...
    async gestisciSelezioneTavolo() {
        ...
        // FIX DEFINITIVO: Se il tavoloManager ha già recuperato il token (tramite aggiorna-stato-tavolo.php),
        // usiamo quello senza chiamare un'altra API che rischierebbe di resettare tutto.
        if (tavolo.sessione_token) {
            this.sessioneCorrente = {
                sessione_token: tavolo.sessione_token
            };
            console.log("Sessione sincronizzata dal Manager:", tavolo.sessione_token);
        } else {
            // Fallback: se per qualche motivo non è nel manager, lo chiediamo al server
        ...
        // Se siamo in modalità "ordine", saltiamo la Fase 2 (QR) e apriamo subito la selezione cliente
    ...
    async generaQRCode() {
        ...
        const urlBase = window.location.origin + '/ristorantemoka/app/home.html';
    ...
    // ⭐ METODO CON PATCH INTEGRATA - Reset clienti completo
    async resettaClientiTavolo() {
        ...
        console.log('[Reset] Resettando clienti tavolo', tavolo.id);

        const result = await window.apiService?.resettaClientiTavolo(tavolo.id);

        if (result && result.success) {
            console.log('[Reset] Reset API OK:', result);
            
            alert(`✅ Clienti del Tavolo ${tavolo.numero} resettati con successo!\n\nClienti cancellati: ${result.clienti_cancellati || result.righe_affected || 0}`);
            
            // ✅ FIX 1: Reset configurazione locale clienti
            if (window.clienteManager && window.clienteManager.clientiConfigurati) {
                console.log('[Reset] Pulisco configurazione locale...');
                
                // Resetta la configurazione per questo tavolo
                if (window.clienteManager.clientiConfigurati[tavolo.id]) {
                    window.clienteManager.clientiConfigurati[tavolo.id] = {
                ...
                // Pulisci anche clientiRegistrati
                window.clienteManager.clientiRegistrati = {};
            }
            
            // ✅ FIX 2: Reset badge UI
            if (window.clienteBadgeManager) {
                console.log('[Reset] Resetto badge UI...');
                window.clienteBadgeManager.resetBadgeClienti();
            }
            
            // ✅ FIX 3: Aggiorna monitor clienti
            console.log('[Reset] Aggiorno monitor...');
            await this.aggiornaMonitorClienti();
            ...
            console.error('[Reset] Errore:', result);
        }
    }
