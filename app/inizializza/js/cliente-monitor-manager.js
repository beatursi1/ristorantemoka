...
if (window.clienteConfigManager && tavolo) {
  ...
  rimuoviCliente(clienteId) {
    ...
    // Se il cliente rimosso è quello che stavamo "prossando", puliamo la sessione
    const proxy = JSON.parse(localStorage.getItem('cameriere_proxy_order') || '{}');
    if (String(proxy.sessione_cliente_id) === String(clienteId)) {
        localStorage.removeItem('cameriere_proxy_order');
    }
...
let tipoTesto = tipoEffettivo === 'manuale'
  ...
  _aggiungiGestoriEventiLista() {
    // FIX: Usiamo .cliente-monitor perché è la classe assegnata in _creaElementoCliente
    const elementi = this.elementiUI.listaClienti.querySelectorAll('.cliente-monitor');
    
    elementi.forEach(elemento => {
      // Click sull'elemento (dettagli)
      elemento.addEventListener('click', (e) => {
        // Se non ho cliccato su un pulsante specifico, seleziono il cliente
        if (!e.target.closest('button')) {
          const clienteId = elemento.dataset.clienteId;
          const tavoloId = elemento.dataset.tavoloId;
          const lettera = elemento.dataset.lettera;

          // --- FIX DISASTRO: Salva sessione cameriere ---
          // Usiamo localStorage (più persistente) e prendiamo il token della sessione
          // FIX TREMENDO: Prendiamo il token della sessione ATTIVA del tavolo, non quello del cameriere
          const tavoloSelezionato = window.tavoloManager?.getTavoloSelezionato();
          // Il token deve essere quello del tavolo specifico (che contiene gli ordini)
          const sessioneTokenReale = tavoloSelezionato?.sessione_token || new URLSearchParams(window.location.search).get('sessione');

          const sessionData = {
            sessione_cliente_id: clienteId,
            sessione: sessioneTokenReale, 
            tavolo: tavoloId,
            cliente: lettera,
            timestamp: Date.now()
          };
          
          // Salviamo sia in sessionStorage che localStorage per massima sicurezza
          localStorage.setItem('cameriere_proxy_order', JSON.stringify(sessionData));
          console.log("Sessione cameriere (proxy) attivata:", sessionData);
          // ----------------------------------------------
      ...
      // Gestione pulsanti interni (se presenti)
      const pulsanti = elemento.querySelectorAll('button');
        ...
        btn.addEventListener('click', (e) => {
          ...
          if (azione) {
            this._emitEvent(`cliente-${azione}`, {
              clienteId: elemento.dataset.clienteId,
              elemento,
              azione
            });
          }
