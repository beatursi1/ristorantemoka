...
(function () {
  ...
  // Apri modal "miei ordini" (accessibility helpers)
    ...
    if (!modalEl.dataset.accessibilityBound) {
      ...
      modalEl.addEventListener('shown.bs.modal', () => {
        ...
        function tryFocus(el) {
          ...
          try { el.focus({ preventScroll: true }); } catch (e) { try { el.focus(); } catch (err) { /* ignore */ } }
        }
      ...
      for (const o of ordini) {
        ...
        if (Array.isArray(o.righe) && o.righe.length) {
          for (const r of o.righe) {
            ...
            left.appendChild(metaDiv);            // ---- BEGIN: render partecipanti (se presenti) ----
            try {
    ...
    if (addBtn && !addBtn.dataset.boundQtyHandler) {
      addBtn.addEventListener('click', () => {
        try {
          ...
          let partecipanti = [];
          if (sharing === 'gruppo') {
            // prova prima '#partecipanti-list .partecipante-checkbox' (modal client-side)
            ...
            partecipanti = boxes.filter(b => b.checked).map(b => b.value || b.dataset.sessioneClienteId || '');
          }
